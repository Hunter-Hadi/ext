 ...
 * @since 2024-01-18
 */
const ArtTextToImageAdvanced: FC<{
  sx?: SxProps
}> = (props) => {
  const { sx } = props
    ...
    return (
      ...
      >
        ...
        >
          <ClickAwayListener
            onClickAway={(event) => {
              const MaxAIArtWithAIAdvancedCard = getMaxAISidebarRootElement()?.querySelector(
                '#MaxAIArtWithAIAdvancedCard',
              ) as HTMLElement
              if (MaxAIArtWithAIAdvancedCard) {
                const rect = MaxAIArtWithAIAdvancedCard.getBoundingClientRect()
...
export default ArtTextToImageAdvanced
